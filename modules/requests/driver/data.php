...
if (!isset($_POST) and $_SERVER['REQUEST_METHOD'] == !"POST") {
  ...
  $requests = DB::columnsmore(["l.name","u.first_name","u.last_name","v.seats"]);
  $requests = DB::table("requests as r");
  $requests = DB::join(["routes AS ro" => ["ro.id" => "r.route_id"]],"LEFT");
  $vehicles = DB::join(["vehicles AS v" => ["v.id" => "ro.vehicle_id"]],"LEFT");
  $requests = DB::join(["locations AS l" => ["l.id" => "ro.location_id"]],"LEFT");
  $requests = DB::join(["users AS u" => ["u.id" => "r.user_id"]],"LEFT");
  $requests = DB::where(["ro.user_id" => app::getSession("id")]);
  ...
  if(isset($_POST["search"])){
    ...
    $requests = DB::and(["ro.user_id" => app::getSession("id")]);
  }
    ...
    foreach ($requests as $request) { ?>
      <tr>
        ...
        <td class="align-middle text-break" data-label="Customer">
          <?php echo ucfirst($request["first_name"])." ".ucfirst($request["last_name"]) ; ?>
        </td>
        ...
        <?php echo $request["fare"] / $request["seats"]; ?> PKR
        </td>
        ...
        <td class="align-middle px-3" data-label="Controls">
          <a class="updateBtn ti-pencil mr-1 font-16 text-success" data-id="<?php echo $request["id"]; ?>" href="javascript:void(0)" title="Update"></a>
          <a class="deleteBtn ti-trash mr-1 font-16 text-danger"  data-id="<?php echo $request["id"]; ?>" href="javascript:void(0)" title="Delete"></a>
        </td>
      </tr>
    <?php } ?>
    <tr id="loader"></tr>
